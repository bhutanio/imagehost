            ...
            @if($images->count())
                @foreach($images as $image)
                    <div class="col-sm-3 block-image">
                        <p><strong><a href="{{ url('i/'.$image->hash) }}" title="{{ ($image->image_title ? $image->image_title : 'Image '.$image->hash) }}">{{ ($image->image_title ? $image->image_title : 'Image '.$image->hash) }}</a></strong></p>
